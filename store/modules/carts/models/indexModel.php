...
function addCartByID($id){
	...
	if(isset($_SESSION['cart']['buy']) && array_key_exists($id, $_SESSION['cart']['buy'])){
		$num_total = $_SESSION['cart']['buy'][$id]['num_total'] + 1;
	}
	...
	$_SESSION['cart']['buy'][$id] = [
		...
		'image' => $item['image'],
		'num_total' => $num_total,
		'sub_total' => $num_total * $item['promotional_price']
	...
	if(!empty($check)){
		...
		$data = ['num_total'=>$num_total ,'sub_total_price'=>$sub_total_price ];
		...
		$id_cart = $tmp['id'];
		$data = ['id_cart '=>$id_cart, 'id_product '=>$id, 'num_total'=>$num_total, 'sub_total_price' =>$sub_total_price];
		...
		if(!empty($dataProductIncart)){
			...
			foreach ($dataProductIncart as $value) {
				...
				$_SESSION['cart']['buy'][$id_product] = [
					...
					'code' => $productes['code'],
					'name' => $productes['name'],
					'promotional_price' => $productes['promotional_price'],
